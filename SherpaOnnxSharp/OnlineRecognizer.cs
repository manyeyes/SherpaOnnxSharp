...
#pragma warning restore CS8602 // 解引用可能出现空引用。
            ...
            _onlineRecognizer =
                DLL.SherpaOnnx.CreateOnlineRecognizer(sherpaOnnxOnlineRecognizerConfig);
        ...
        {
            SherpaOnnxOnlineStream stream = DLL.SherpaOnnx.CreateOnlineStream(_onlineRecognizer);
            return new OnlineStream(stream);
        }
        public void InputFinished(OnlineStream stream)
        {
            DLL.SherpaOnnx.InputFinished(stream._onlineStream);
        }
        ...
        {
            DLL.SherpaOnnx.AcceptOnlineWaveform(stream, sample_rate, samples, samples.Length);
        }
        ...
        {
            return DLL.SherpaOnnx.IsOnlineStreamReady(_onlineRecognizer, stream._onlineStream) != 0;
        }
        public void DecodeMultipleStreams(List<OnlineStream> streams)
        {
            while (true)
            {
                ...
                DLL.SherpaOnnx.DecodeMultipleOnlineStreams(_onlineRecognizer, streamsIntPtr, streamsBatch.Length);
        ...
        public void DecodeStream(OnlineStream stream)
        {
            ...
            {
                DLL.SherpaOnnx.DecodeOnlineStream(_onlineRecognizer, stream._onlineStream);
            }
        }

        internal OnlineRecognizerResultEntity GetResult(SherpaOnnxOnlineStream stream)
        {
            IntPtr result_ip = DLL.SherpaOnnx.GetOnlineStreamResult(_onlineRecognizer, stream);
